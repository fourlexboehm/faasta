...
impl GitHubAuth {
    ...
    pub async fn authenticate(&mut self) -> Result<(), Error> {
        if self.config.app_id == 0 || self.config.installation_id == 0 || self.config.private_key.is_empty() {
            return Err(anyhow::anyhow!("GitHub App not configured. Run 'cargo faasta auth setup' first."));
        }
        
        self.token = Some(InstallationAccessToken::new(GithubAuthParams {
    ...
    pub async fn store_project_hmac(&mut self, project_name: &str, hmac: &str) -> Result<(), Error> {
        self.config.project_hmacs.insert(project_name.to_string(), hmac.to_string());
    ...
    pub async fn setup(&mut self, app_id: u64, installation_id: u64, private_key: Vec<u8>) -> Result<(), Error> {
    ...
    fn get_config_path() -> Result<PathBuf, Error> {
        let mut path = config_dir().ok_or_else(|| {
            anyhow::anyhow!("Could not find user config directory")
        })?;
